...
def edit_selected_data():
    ...
    if (len(selected_indices) == 1 and len(rselected_indices) == 0) or (len(selected_indices) == 0 and len(rselected_indices) == 1):
    # 左側リストで選択されたイベントが1つなら
        if len(selected_indices) == 1:
            selected_event_id = selected_indices[0]  # 選択されたイベントIDを取得
            service = get_calendar_service()  # Google Calendarサービスを取得
            selected_event = service.events().get(calendarId='primary', eventId=selected_event_id).execute()  # イベント情報を取得
        else:
            # 右側リストで選択されたイベントが1つなら
            selected_event_id = rselected_indices[0]  # 選択されたイベントIDを取得
            service = get_calendar_service()  # Google Calendarサービスを取得
            selected_event = service.events().get(calendarId='primary', eventId=selected_event_id).execute()  # イベント情報を取得
        ...
        ctk.CTkLabel(root, text="編集するためには、左右のリストからそれぞれ一つのタスクを選択してください。", text_color="red", font=("Helventic",16)).pack(pady=10)
